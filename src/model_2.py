...
def build_time_indexed_milp(data, time_slots):
    ...
    # Build instructor-session mappings
...
def solve_and_extract_schedule(solver, variables, data, time_slots):
    """
    Solve the model and extract a human-readable schedule
    """
    status = solver.Solve()
    ...
    print(f"Objective value (total attendance): {solver.Objective().Value()}")
    ...
    for s in session_ids:
        ...
        if start_time is not None:
            end_time = start_time + durations_in_slots[s]
            
            # Find assigned room (for in-person sessions)
            assigned_room = None
            if s in [sid for sid in session_ids if data["sessions"][data["sessions"]["Session ID"] == s]["Modality"].iloc[0] == "in-person"]:
                room_ids = data["rooms"]["Room ID"].tolist()
                for r in room_ids:
                    if y.get((s, r)) is not None and y[(s, r)].solution_value() > 0.5:
                        assigned_room = r
                        break
            ...
            schedule.append({
                ...
                'Duration (slots)': durations_in_slots[s],
                'Assigned Room': assigned_room if assigned_room else 'Virtual'
            })
    
    return pd.DataFrame(schedule).sort_values('Start Time Slot')

# Example usage function
def run_optimization_example():
    """
    Example of how to use the time-indexed model
    """
    # This would be replaced with actual data loading
    # For now, showing the structure
    
    # Create sample time slots (90 days * 6 half-hour slots per day = 540 slots)
    time_slots = list(range(540))
    
    # Sample data structure - replace with actual data loading
    sample_data = {
        "sessions": pd.DataFrame({
            "Session ID": ["S1", "S2", "S3"],
            "Duration (mins)": [60, 90, 30],
            "Order": [1, 2, 3],
        ...
        "group_training": pd.DataFrame({
            "Group ID": ["G1", "G1", "G2", "G2"],
            "Session ID": ["S1", "S2", "S1", "S3"]
        }),
        "instructor_assignments": pd.DataFrame({
            "Instructor ID": ["I1", "I2", "I1"],
            "Session ID": ["S1", "S2", "S3"]
        })
    }
    
    # Build and solve model
    solver, variables = build_time_indexed_milp(sample_data, time_slots)
    schedule = solve_and_extract_schedule(solver, variables, sample_data, time_slots)
